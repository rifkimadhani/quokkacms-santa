...
$htmlNew = $form->renderDialog('New VOD', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete vod', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        //
        var genreoptions = {placeholder: "Pilih Genre Disini",allowClear: true,tags:true,"language": {"noResults": function(){return "No Genre Found.Please Type Genre Name To Add New Genre";}},escapeMarkup: function (markup) {return markup;}}
        ...
        {   
            ...
            $.ajax({url: url}).done(function(result)
            {
                ...
                // find the select element within the dialogformEdit and initialize it as a Select2 element
                var selectElement = $('.dialogformEdit').find('.js-example-basic-multiple');
                if (selectElement.length) {
                    selectElement.select2(genreoptions)
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + vodId;
        })
    }
</script>
