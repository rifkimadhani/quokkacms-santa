...
$htmlNew = $form->renderDialog('New currency', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete currency', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            const url = "<?=$baseUrl?>/edit/" + currency;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + currency;
        })
    }
</script>