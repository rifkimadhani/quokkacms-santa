...
$htmlNew = $form->renderDialog('New inbox', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete inbox', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const inboxId = data[0];

            const url = "<?=$baseUrl?>/edit/" + inboxId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + inboxId;
        })
    }
</script>