...
$htmlNew = $form->renderDialog('New roomtype', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete roomtype', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const roomTypeId = data[0];

            const url = "<?=$baseUrl?>/edit/" + roomTypeId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + roomTypeId;
        })
    }
</script>