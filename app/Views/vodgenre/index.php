...
$htmlNew = $form->renderDialog('New vodgenre', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete vodgenre', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            const url = "<?=$baseUrl?>/edit/" + genreId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + genreId;
        })
    }
</script>