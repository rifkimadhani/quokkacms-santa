...
$htmlNew = $form->renderDialog('New language', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete language', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const langCode = data[0];

            const url = "<?=$baseUrl?>/edit/" + langCode;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + langCode;
        })
    }
</script>