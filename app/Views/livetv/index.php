...
$htmlNew = $form->renderDialog('New livetv', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete livetv', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        dataTable = dataList.DataTable(
            {
                ...
                columnDefs: [
                    ...
                    //         return '<div style="width:256px; overflow: hidden; text-overflow: ellipsis;">' + data + '</div>';
                    //     }
                    // },
                    {
                        ...
                        {        
                            if(data)
                            {
                                ...
                                return '<img src="'+data+'" height="50px" class="urlimage">';
                            }
                            return '<img src="<?= base_url("assets/notfound.png") ?>" width="70px" height="50px" class="urlimage">';
                        },
                        width:100
                    },
                    {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const livetvId = data[0];

            const url = "<?=$baseUrl?>/edit/" + livetvId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + livetvId;
        })
    }
</script>