...
$htmlNew = $form->renderDialog('New kitchen', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete kitchen', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const kitchenId = data[0];

            const url = "<?=$baseUrl?>/edit/" + kitchenId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + kitchenId;
        })
    }
</script>