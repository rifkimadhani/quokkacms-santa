 ...
 * Date: 2023-06-07 15:39:06
 */

use App\Libraries\Dialog;

// $htmlEdit = $form->renderPlainDialog('formEdit');
// $htmlNew = $form->renderDialog('New Emergency History', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete Emergency History', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const emergencyHistoryId = data[0];

            const url = "<?=$baseUrl?>/edit/" + emergencyHistoryId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + emergencyHistoryId;
        })
    }
</script>