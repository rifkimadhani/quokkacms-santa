...
$htmlNew = $form->renderDialog('New livetvcategory', 'formNew', "{$baseUrl}/insert");
$htmlDelete = Dialog::renderDelete('Delete livetvcategory', 'CONFIRM DELETE');
?>
...
<script>
    ...
    function initDataTable() {
        ...
        dataList.find('tbody').on( 'click', 'tr', function (event)
        {
            ...
            //get pk from data
            const livetvCategoryId = data[0];

            const url = "<?=$baseUrl?>/edit/" + livetvCategoryId;
    ...
    function onClickTrash(event, that) {
        ...
        showDialogDelete('formDelete', 'Are you sure to delete ' + name, function () {
            window.location.href = "<?=$baseUrl?>/delete/" + livetvCategoryId;
        })
    }
</script>